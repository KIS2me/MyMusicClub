...
export default {
    ...
    watch:{
        ...
        select_word: function(){
            if(this.select_word === ''){
                ...
                // getSingerLikeName(name).then(res => {
    ...
    methods:{
        ...
        getData(){
            ...
            this.tableData = [];
            // //分页查询歌手
            // getSingersForPage(this.currentPage)
            //     .then(res => {  /*res为从后台数据库查询到的数据*/
            //         // this.tempData = res;
            //         this.tableData = res;
            // })
            //查询所有歌手
        ...
        addSinger(){
            ...
            setSinger(params)
            .then(res => {  /*res表示从后台返回的值，即后台controller层执行insert方法后返回的jsonObject*/
                if(res.code === 1){
                    this.getData();
